...
function us_demo_import_content_all() {
	...
	if ( copy( $file_url, $upload_dir['basedir'] . '/all_content.xml' ) AND filesize( $upload_dir['basedir'] . '/all_content.xml' ) > 50 ) {
		$content_file = $upload_dir['basedir'] . '/all_content.xml';
	} else {
		wp_send_json(
			array(
				'success' => FALSE,
				'error_title' => sprintf( __( 'Failed to import %s', 'us' ), us_translate_with_external_domain( 'All content' ) ),
				'error_description' => __( 'Wrong path to the file or it is missing.', 'us' ),
			)
		);
	}

	us_demo_import_content( $content_file );
	unlink( $content_file );
...
function us_demo_import_content_pages() {
	...
	$upload_dir = wp_upload_dir();
	if ( copy( $file_url, $upload_dir['basedir'] . '/pages.xml' ) AND filesize( $upload_dir['basedir'] . '/pages.xml' ) > 50 ) {
		$content_file = $upload_dir['basedir'] . '/pages.xml';
	} else {
		wp_send_json(
			array(
				'success' => FALSE,
				'error_title' => sprintf( __( 'Failed to import %s', 'us' ), us_translate_with_external_domain( 'Pages' ) ),
				'error_description' => __( 'Wrong path to the file or it is missing.', 'us' ),
			)
		);
	}
...
function us_demo_import_content_posts() {
	...
	$upload_dir = wp_upload_dir();
	if ( copy( $file_url, $upload_dir['basedir'] . '/posts.xml' ) AND filesize( $upload_dir['basedir'] . '/posts.xml' ) > 50 ) {
		$content_file = $upload_dir['basedir'] . '/posts.xml';
	} else {
		wp_send_json(
			array(
				'success' => FALSE,
				'error_title' => sprintf( __( 'Failed to import %s', 'us' ), us_translate_with_external_domain( 'Posts' ) ),
				'error_description' => __( 'Wrong path to the file or it is missing.', 'us' ),
			)
		);
	}
...
function us_demo_import_content_portfolio() {
	...
	$upload_dir = wp_upload_dir();
	if ( copy( $file_url, $upload_dir['basedir'] . '/portfolio_items.xml' ) AND filesize( $upload_dir['basedir'] . '/portfolio_items.xml' ) > 50 ) {
		$content_file = $upload_dir['basedir'] . '/portfolio_items.xml';
	} else {
		wp_send_json(
			array(
				'success' => FALSE,
				'error_title' => sprintf( __( 'Failed to import %s', 'us' ), __( 'Portfolio Items', 'us' ) ),
				'error_description' => __( 'Wrong path to the file or it is missing.', 'us' ),
			)
		);
	}
...
function us_demo_import_content_testimonials() {
	...
	$upload_dir = wp_upload_dir();
	if ( copy( $file_url, $upload_dir['basedir'] . '/testimonials.xml' ) AND filesize( $upload_dir['basedir'] . '/testimonials.xml' ) > 50 ) {
		$content_file = $upload_dir['basedir'] . '/testimonials.xml';
	} else {
		wp_send_json(
			array(
				'success' => FALSE,
...
function us_demo_import_widgets() {
	...
	$upload_dir = wp_upload_dir();
	if ( copy( $file_url, $upload_dir['basedir'] . '/widgets.json' ) AND filesize( $upload_dir['basedir'] . '/widgets.json' ) > 50 ) {
		$content_file = $upload_dir['basedir'] . '/widgets.json';
		ob_start();
		require_once( $us_template_directory . '/framework/vendor/widget-importer-exporter/import.php' );
		us_wie_process_import_file( $content_file );
		ob_end_clean();
		unlink( $content_file );
...
function us_demo_import_woocommerce() {
	...
	$upload_dir = wp_upload_dir();
	if ( copy( $file_url, $upload_dir['basedir'] . '/products.xml' ) AND filesize( $upload_dir['basedir'] . '/products.xml' ) > 50 ) {
		$content_file = $upload_dir['basedir'] . '/products.xml';
	} else {
		wp_send_json(
			array(
				'success' => FALSE,
...
function us_demo_import_options() {
	...
	$upload_dir = wp_upload_dir();
	if ( copy( $file_url, $upload_dir['basedir'] . '/theme-options.json' ) AND filesize( $upload_dir['basedir'] . '/theme-options.json' ) > 50 ) {
		$content_file = $upload_dir['basedir'] . '/theme-options.json';
	} else {
		wp_send_json(
			array(
				'success' => FALSE,
	...
	$updated_options = json_decode( file_get_contents( $content_file ), TRUE );
...
function us_demo_import_sliders() {
	...
	if ( count( $sliders ) > 0 ) {
		foreach ( $sliders as $slider_filename ) {
			...
			$upload_dir = wp_upload_dir();
			if ( copy( $file_url, $upload_dir['basedir'] . '/' . $slider_filename ) AND filesize( $upload_dir['basedir'] . '/' . $slider_filename ) > 50 ) {
				$content_file = $upload_dir['basedir'] . '/' . $slider_filename;
			} else {
				wp_send_json(
					array(
						'success' => FALSE,
...
//			ob_start();
			$_FILES["import_file"]["tmp_name"] = $content_file;
