...
function us_load_metabox_header_settings( $header_settings ) {
	if ( is_singular( array( 'post', 'page', 'us_portfolio', 'product' ) ) OR ( is_404() AND $page_404 = get_page_by_path( 'error-404' ) ) ) {
		if ( is_singular() ) {
			$postID = get_the_ID();
		} elseif ( is_404() ) {
			$postID = $page_404->ID;
		...
		foreach ( array( 'default', 'tablets', 'mobiles' ) as $state ) {
			if ( us_arr_path( $header_settings, $state . '.options.orientation' ) == 'hor' ) {
				if ( usof_meta( 'us_header_bg', array(), $postID ) != '' ) {
					$header_settings[$state]['options']['transparent'] = intval( usof_meta( 'us_header_bg', array(), $postID ) == 'transparent' );
				}
				if ( usof_meta( 'us_header_pos', array(), $postID ) != '' ) {
					$header_settings[$state]['options']['sticky'] = intval( usof_meta( 'us_header_pos', array(), $postID ) == 'fixed' );
				}
			}
		}
	}
...
/**
 * Add no-touch class to body on mobiles
 */
