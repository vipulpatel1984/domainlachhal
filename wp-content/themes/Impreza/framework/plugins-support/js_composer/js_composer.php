	...
	function us_testimonials_ids_autocomplete_suggester ( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = $value['id'] . ' - ' . ( ( strlen( $value['title'] ) > 0 ) ? $value['title'] : us_translate_with_external_domain( '(no title)' ) );
				$results[] = $data;
			}
		}
	...
	function us_testimonials_ids_render( $query ) {
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $post_object ) ) {
				...
				$data = array();
				$data['value'] = $post_id;
				$data['label'] = $post_id . ' - ' . ( ( strlen( $post_title ) > 0 ) ? $post_title : us_translate_with_external_domain( '(no title)' ) );
